...
Phase 2: persisted via the repo, generation still stubbed. Phase 3 swaps in D's
real functions: generate_voice_variants(base_media_key, [spec.model_dump()...]),
gemini.suggest(base_media_key, context or ""), llm.tips(user_id) (+ persist
users.backboard_thread_id).
"""
import string
...
async def voice_variants(test_id: str, body: VoiceVariantsReq, user=Depends(current_user)):
    ...
    for i, spec in enumerate(body.variants):
        ...
        variant = {
            ...
            # Phase 2 stub: no real mux — point at the base video so the UI can play it.
    ...
    await repo().update_test(
        test_id,
        {"variant_ids": test["variant_ids"] + new_ids, "updated_at": now_iso()},
    )
...
async def suggest(body: SuggestReq, user=Depends(current_user)):
    """Phase-2 canned test plan. Phase 3: D's gemini.suggest watches the video (CONTRACTS §5)."""
    return {
        "variants": [
...
async def tips(test_id: str, user=Depends(current_user)):
    """Phase-2 canned tips. Phase 3: D's llm.tips(user_id) via Backboard memory."""
