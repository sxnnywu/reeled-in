...
The slowest, riskiest task on the whole project is **A's real loop runner** (Gemini agents executing the FinFlow topology, fan-out, Backboard memory). Almost everything downstream — B's collector/WebSocket, C's pipelines/findings/report, D's live feed and report view — needs *a stream of contract-shaped events to build against.*

If everyone waits for the real runner, the team serializes behind A. **Solution:** A builds a tiny **fake emitter** early — a ~50-line script that generates `Event` documents (agent_message, tool_call, loop_iteration, termination incl. `stall_detected`, etc.) for a fake run and POSTs them to B's collector on a timer, including a deliberately planted stall cluster.

The moment fake events flow into Mongo, **B, C, and D are all unblocked in parallel** and can build and test their real logic against realistic data — while A goes off and builds the real runner separately. Then in Phase 3 you swap the fake source for the real FinFlow runner. This single decoupling is what turns a mostly-serial project into a mostly-parallel one.
...
A: real loop runner + FinFlow ─► real data (swap in for fake emitter in Phase 3)
```
...
| 10 | Real FinFlow loop runner produces real events | A | Real demo data (swap out fake) | H8–H14 |
...
- **A:** the real work — fan-out controller + lifecycle state machine, loop runner executing the FinFlow topology with Gemini agents, Backboard memory (`state_update` events), batched real event emission. Then FinFlow example spec + seed variation.
...
- **A ↔ B/C:** swap the fake emitter for the real FinFlow runner as the event source. Confirm real events match the schema exactly (they should — same models).
...
- **Backboard** — agent memory/state inside the test loops (already core). ✔
- **MongoDB (MLH)** — event store + aggregations (already core). ✔
- **Gemini (MLH)** — loop agents + finding summarization (already core). ✔
