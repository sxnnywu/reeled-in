...
- **D [PARALLEL]:** `generation/variants.py` (finalize `generate_voice_variants` ✅ DONE in P1), `generation/llm.py` (Backboard suggestions + memory/RAG), `generation/explainer.py` (region_timeline → per-second captions). `gemini.py` ✅ DONE — reworked per CONTRACTS change: Gemini *watches* the base video (multimodal, inline mp4 → `gemini-2.5-flash-lite`) and returns `{variants: [VoiceSpec], rationale}`; full funnel verified live (Gemini plan → rendered variants).
